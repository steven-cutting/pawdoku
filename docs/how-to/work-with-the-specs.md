...
| `technique.allium` | The shared catalogue of 29 named techniques, proof obligations and acceptance schemas; the grid and coarse profile used by the baseline models. It imports `sudoku.allium` alone. |
| `reach.allium` | The player model in which a limit hides a deduction: what a profile sees, which deduction it takes next, how far it gets through a set of givens, and the hint it would be given in a puzzle in play. It imports `sudoku.allium` and `technique.allium` and neither of the other two models. |
...
| `lapse.allium` | The baseline model in which a limit leads to error: upkeep of marks, checks, guesses and repairs, rated by steps. Coarse and deterministic. Same imports as `reach.allium`. |
| `human-solving.allium` | Bounded attention and memory, independent experience, fallible beliefs and notes, explicit effort and seeded attempts, plus profile-relative repeated assessment. Owns its richer profile and four presets. Imports rules and techniques; shares no lifecycle with the three baseline models. |
