...
| `lapse.allium` | The player model in which a limit leads to error: upkeep of marks, checks, guesses and repairs, rated by steps, asked for by `Tackle`. Coarse and deterministic. Same imports as `reach.allium`. |
| `human-solving.allium` | Bounded attention and memory, independent experience, fallible beliefs and notes, explicit effort and seeded attempts, plus profile-relative repeated assessment. The fourth player model and the one with chance in it, drawn through the randomness port. Owns its finer profile, four presets and the projection onto `technique.allium`'s profile. Same imports as `reach.allium`. |
