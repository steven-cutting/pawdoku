...
| `technique.allium` | What a person can deduce: the catalogue of named techniques in ladder order, the grid, units and candidates they are read from, exactly when each technique through subsets holds and what it places or strikes, and the profile of a player with its novice and expert presets. It imports `sudoku.allium` alone and owns the grid every player model keeps. |
...
| `lapse.allium` | The player model in which a limit leads to error: upkeep of marks, checks, guesses and repairs, rated by steps. Coarse, and where a stochastic layer would later sit. Same imports as `reach.allium`. |
